...
|---|-------|----------|----------|
| B-50 | App-Updates: Build-ID + /api/version, Einstellungen (auto/nachfragen/manuell, Was ist neu), Admin-Releases inkl. Pflicht-Update + Feature-Freischaltung, Migration 0005 | API 24/24 ✅ · vitest ✅ · Update-Erkennung live mit zwei Builds ✅ (siehe TESTING.md) | Claude (Anthropic) |
| B-49 | Globale Feature-Flags (DB + 403-Sperre + Polling/Fokus/PTR), echtes Pull-to-Refresh (leise, alle Ressourcen, SW-Update), Migration 0004 | API 23/23 ✅ · vitest 76/76 ✅ · Validierung siehe TESTING.md | Claude (Anthropic) |
| B-48 | UI/UX: Slider, Gesten-Zonen, Remember-Me, Logout, Badges/Glocke, Notification-Links, PWA-Install + Icons, Forum-Votes/Antworten, Hall/Social-Kommentare, Migration 0003 | API 22/22 ✅ · vitest 71/71 ✅ · Validierung siehe TESTING.md | Claude (Anthropic) |
| B-47 | Media in PostgreSQL + Uploads (Social/Chat/Galerie/Avatar), Profil-Editor, Emoji, Teilen, Sammlung, Deep-Links, Migration 0002 | API 20/20 ✅ · vitest 64/64 ✅ · Validierung siehe TESTING.md | Claude (Anthropic) |
| B-46 | Embedded-API unter /api, Auto-Migration, API-Modus Standard, Sandbox-DB migriert + Seed | Persistenz nach Neustart ✅ · 37 Endpunkte gegen echte DB ✅ · Validierung siehe TESTING.md | Claude (Anthropic) |
| B-45 | Paket 3 (Service-Layer statt Mocks, Typen, Lazy-Views, Audit) + API-Routen strains/wiki/tasks + Migration 0001 | typegen ✅ tsc ✅ lint ✅ vitest ✅ next build ✅ · API typecheck/build ✅ · API-Integration 15/15 ✅ · Migration 0000→0001 mit Daten ✅ · npm audit Frontend 0 | Claude (Anthropic) |
| B-44 | Gesten im ganzen Panel + Toasts + Tab-Wischen, View `create` (5 Schnellaktionen), Vollbild-Galerie, Sortenvergleich, Hall-of-Fame-Karte | typegen ✅ tsc ✅ lint ✅ vitest 61/61 ✅ next build ✅ (siehe TESTING.md) | Claude (Anthropic) |
| B-43 | Paket 2: tote Dateien/Exporte entfernt (`InlineIcon.tsx`, `pnpm-workspace.yaml`, doppeltes `public/images/hero.jpg`, `checkBackend`, `simulationCurve`), Test-/Lint-Tools → devDependencies, CI auch auf Branch `A`, PLAN/MIGRATION/TODO mit B-42-Status | siehe TESTING.md (B-43) | Claude (Anthropic) |
| B-42 | API-Blocker (migrate.ts, Install-Abbruch), Next.js-SPA-Hülle + `/api/health`, `src/pages`→`src/views`, SW-Policy für `/_next/static`, react-hooks-Lint, `.gitignore` | **Ausgeführt:** typegen ✅ tsc ✅ lint 0/0 ✅ vitest 52/52 ✅ next build ✅ vite build ✅ (870 kB) · API typecheck/tools/build ✅ · API-Integration 13/13 ✅ | Claude (Anthropic) |

...
/* ----------------------------- Grow-Simulation ----------------------------- */
export const simulationCurve = Array.from({ length: 13 }, (_, w) => {
  // Logistisches Wachstum über 12 Wochen
  const k = 0.55;
  const height = +(180 / (1 + Math.exp(-k * (w - 5))) + 18).toFixed(0);
  const canopy = +(100 / (1 + Math.exp(-0.7 * (w - 4)))).toFixed(0);
  const phase = w < 1 ? "Keim" : w < 5 ? "Veg" : w < 11 ? "Blüte" : "Ernte";
  return { week: w, height, canopy, phase };
});

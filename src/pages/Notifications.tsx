...
export default function Notifications() {
  ...
  const markAll = () => { setItems((s) => s.map((n) => ({ ...n, read: true }))); toast.push({ title: "Alle als gelesen markiert", tone: "leaf", icon: "CheckCircle2" }); };
  const unread = items.filter((n) => !n.read).length;

  return (
    <div className="space-y-6">
      <PageHeader title="Benachrichtigungen" subtitle={`${unread} ungelesen · bleib auf dem Laufenden.`} icon="Bell"
        actions={<Button variant="secondary" onClick={markAll} disabled={unread === 0}><Icon name="CheckCircle2" size={16} /> Alle gelesen</Button>} />
        ...
        <div className="space-y-2">
          {list.map((n, i) => (
            <Reveal key={n.id} delay={i * 0.03}>
              <button
                onClick={() => { setItems((s) => s.map((x) => (x.id === n.id ? { ...x, read: true } : x))); toast.push({ title: n.title, desc: n.body, tone: typeTone[n.type], icon: typeIcon[n.type] }); }}

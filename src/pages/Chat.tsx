...
import { Avatar, Card, EmptyState, PageHeader, SearchInput } from "@/components/ui";
import { useToast } from "@/components/Toast";
import { conversations, type ChatMessage, type Conversation } from "@/mocks/data";

export default function Chat() {
  const [sel, setSel] = useState<string | null>(conversations[0].id);
  const conv = conversations.find((c) => c.id === sel) ?? null;
      ...
      <Card className="grid h-[calc(100dvh-16rem)] min-h-[420px] grid-cols-1 overflow-hidden p-0 sm:h-[calc(100vh-220px)] lg:grid-cols-[320px_1fr]">
        ...
        <div className={cn("flex flex-col border-border lg:border-r", conv && "hidden lg:flex")}>
          <div className="border-b border-border p-3">
            <SearchInput placeholder="Konversation suchen…" />
          </div>
          <div className="flex-1 overflow-y-auto">
            {conversations.map((c) => (
              <button key={c.id} onClick={() => setSel(c.id)} className={cn("flex w-full items-center gap-3 p-3 text-left transition hover:bg-surface-2", sel === c.id && "bg-surface-2")}>
        ...
        <div className={cn("flex flex-col", !conv && "hidden lg:flex")}>
          {conv ? <ChatWindow key={conv.id} conv={conv} onBack={() => setSel(null)} /> : <div className="grid flex-1 place-items-center p-6"><EmptyState icon="MessageCircle" title="Kein Chat ausgewählt" desc="Wähle links eine Konversation, um zu starten." /></div>}
        </div>
      </Card>
    </div>
  );
...
function ChatWindow({ conv, onBack }: { conv: Conversation; onBack: () => void }) {
  ...
  const [msgs, setMsgs] = useState<ChatMessage[]>(conv.messages);
  ...
  const send = () => {
    if (!text.trim()) return;
    setMsgs((m) => [...m, { id: String(Date.now()), from: "me", text: text.trim(), time: "jetzt" }]);
    setText("");
  };

  const typing = conv.last === "Tippt gerade…";

  return (
    <>
      <div className="flex items-center gap-3 border-b border-border p-3">
        <button onClick={onBack} className="grid size-9 place-items-center rounded-lg text-fg-muted hover:bg-surface-2 lg:hidden" aria-label="Zurück"><ArrowLeft className="size-5" /></button>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-4">
        ...
        {typing && (
          <div className="flex items-center gap-1.5 rounded-2xl rounded-bl-sm bg-surface-2 px-4 py-3 w-fit">
            {[0, 1, 2].map((i) => (
      ...
      <div className="flex items-center gap-2 border-t border-border p-3">
        ...
        <button onClick={send} className="grid size-10 shrink-0 place-items-center rounded-xl bg-accent text-accent-fg transition hover:brightness-110" aria-label="Senden"><Send className="size-5" /></button>
      </div>
    </>
  );
}

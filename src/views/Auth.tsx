...
export default function Auth() {
  ...
  const [name, setName] = useState(config.useMock ? "Max Grünfeld" : "");
  ...
  const submit = () => {
    ...
    if (mode === "register" && !terms) {
      toast.push({ title: "Zustimmung fehlt", desc: "Bitte AGB & Datenschutz akzeptieren.", tone: "warning", icon: "AlertTriangle" });
      return;
    }
    setBusy(true);
    const run = mode === "login" ? login(email, password, remember) : register(name, email, password);
    run
      .then(() => {
          ...
          <Card className="card glass p-6 elev-2 sm:p-8">
            ...
            <div className="mt-6 space-y-4">
              ...
              {mode === "login" ? (
                ...
                <Checkbox checked={terms} onChange={setTerms} label="Ich akzeptiere die AGB & Datenschutz" />
              )}

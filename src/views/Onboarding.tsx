...
import { installHint } from "@/lib/pwa";
import heroAsset from "@/assets/hero.jpg";
import { assetSrc } from "@/lib/asset";

const heroImg = assetSrc(heroAsset);
...
export default function Onboarding({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const next = async () => {
    ...
    const done = await install.run();
    // Bei Anleitung (iOS/manuell) bleibt der Dialog offen, damit der Hinweis gelesen werden kann.
    if (done || install.state === "prompt") onClose();
  };
            ...
            <div className="flex min-h-full items-center justify-center p-4 pb-8">
              ...
              >
                ...
                <div className="mt-6 flex gap-2 sm:mt-7">
                  ...
                  <Button className="flex-1" onClick={() => void next()} loading={install.busy}>
                    {step === 0 ? "Weiter" : install.state === "installed" ? "Los geht's" : "App installieren"} <ArrowRight className="size-4" />
                  </Button>
                </div>
                {step === 1 && install.state !== "prompt" && install.state !== "installed" && (
                  <p className="mt-3 rounded-xl bg-white/10 p-3 text-center text-xs text-white/80">{installHint(install.state)}</p>
                )}
                <button onClick={onClose} className="mt-3 w-full text-center text-sm text-white/50 hover:text-white/80">
                  {step === 1 ? "Später" : "Überspringen"}
                </button>
              </motion.div>
            </div>
          </div>

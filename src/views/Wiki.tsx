...
import { AVATARS } from "@/mocks/data";
import { useWikiCategories } from "@/data/hooks";
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Wiki & Wissensbibliothek" subtitle="Community-gepflegtes Wissen – editierbar wie ein Code-Repo." icon="BookOpen"
        actions={<Button variant="secondary" onClick={() => navigate("create", { kind: "wiki" })}><Pencil className="size-4" /> Artikel erstellen</Button>} />
      <Reveal>
        <Card className="space-y-3 p-4">
          <SearchInput placeholder="Artikel durchsuchen…" value={q} onChange={(e) => setQ(e.target.value)} />
          <Tabs value={cat} onChange={setCat} tabs={(categories.length ? categories : ["Übersicht"]).map((c) => ({ value: c, label: c }))} />
        </Card>
      </Reveal>
